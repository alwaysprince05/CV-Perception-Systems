...
# ─────────────────────────────────────────────────────────────────────────────
#  3D PERCEPTION PANEL  (Matplotlib) — NEON STYLE FIX
# ─────────────────────────────────────────────────────────────────────────────
def build_3d_panel(tracks, depth_map, panel_w, panel_h):
    """Return an np.uint8 BGR image of the 3D perception panel — full neon style."""
    dpi = 100
    fig_w = panel_w / dpi
    fig_h = panel_h / dpi
    ...
    # ── Make all pane backgrounds transparent / dark ──────────────────────────
    for pane in [ax.xaxis.pane, ax.yaxis.pane, ax.zaxis.pane]:
        pane.fill = True
        pane.set_facecolor((0.02, 0.04, 0.06, 0.85))
        pane.set_edgecolor((0.0, 0.6, 0.3, 0.25))

    # ── Neon ground grid — draw manually for full color control ───────────────
    grid_x = np.linspace(-14, 14, 22)
    grid_z = np.linspace(0, 85, 32)

    # Longitudinal lines (along depth)
    for gx in grid_x:
        ax.plot([gx, gx], [0, 85], [0, 0],
                color=(0.0, 0.55, 0.25), linewidth=0.5, alpha=0.55)

    # Lateral lines (across width)
    for gz in grid_z:
        # Colour lines by depth: closer = brighter cyan, far = dim green
        t_depth = gz / 85.0
        r = 0.0
        g = 0.55 + 0.45 * (1 - t_depth)   # bright near, dim far
        b = 0.4  * (1 - t_depth)
        ax.plot([-14, 14], [gz, gz], [0, 0],
                color=(r, g, b), linewidth=0.5, alpha=0.5)
    ...
    if depth_map is not None:
        ...
        if len(z_v) > 0:
            # plasma colormap: close=yellow/red, far=blue/purple — VERY neon
            colors = plt.cm.plasma(1.0 - d_v)
            ax.scatter(x_v, z_v, y_v, c=colors, s=0.8, alpha=0.65, linewidths=0)

    # ── 3D Vehicle boxes ──────────────────────────────────────────────────────
    BOX_COLORS = {
        "close":  ((1.0, 0.15, 0.05), (1.0, 0.5, 0.0)),   # red-orange for <15m
        "medium": ((1.0, 0.55, 0.0),  (1.0, 0.85, 0.0)),  # orange-yellow for <30m
        "far":    ((0.0, 1.0, 0.45),  (0.0, 0.8, 1.0)),   # green-cyan for far
    }
    ...
    for t in tracks:
        ...
        if d < 15:
            edge_col, glow_col = BOX_COLORS["close"]
        elif d < 30:
            edge_col, glow_col = BOX_COLORS["medium"]
        else:
            edge_col, glow_col = BOX_COLORS["far"]
        ...
        for a, b in edges:
            ax.plot([corners[a,0], corners[b,0]],
                    ...
                    color=glow_col, lw=3.5, alpha=0.18)

        # Draw sharp neon pass
        for a, b in edges:
            ax.plot([corners[a,0], corners[b,0]],
                    [corners[a,1], corners[b,1]],
                    [corners[a,2], corners[b,2]],
                    color=edge_col, lw=1.4, alpha=0.95)

        # Vertical base line from box to ground
        ax.plot([wx, wx], [wz, wz], [0, 0],
                color=glow_col, lw=1.0, alpha=0.4, linestyle=':')

        # Trajectory trail
        if len(t.history) >= 2:
            hist = list(t.history)
            # project 2D history to a rough world path
            hist_wx = [img_cx_norm * max(t.dist - (len(hist)-1-i)*0.4, 1) * 0.5 for i in range(len(hist))]
            hist_wz = [max(t.dist - (len(hist)-1-i)*0.4, 1) for i in range(len(hist))]
            ax.plot(hist_wx, hist_wz, [0.05]*len(hist),
                    color=(0.0, 1.0, 1.0), lw=1.0, alpha=0.55, linestyle='--')

        # Distance label with neon glow effect
        label_col = '#ff3322' if d < 15 else ('#ffaa00' if d < 30 else '#00ffaa')
        ax.text(wx, wz, ch + 0.6,
                f"#{t.id} {t.dist:.1f}m",
                color=label_col, fontsize=6.0, ha='center',
                fontweight='bold',
                bbox=dict(boxstyle='round,pad=0.15', facecolor=(0,0,0,0.55),
                          edgecolor=label_col, linewidth=0.6))

    # ── Ego vehicle indicator ─────────────────────────────────────────────────
    ax.scatter([0], [0], [0], c=[[1.0, 1.0, 0.0]], s=80, marker='^', zorder=10,
               edgecolors=(1,0.8,0,0.9), linewidths=1.2)
    ax.text(0, -4, 0.8, "EGO", color='#ffff00', fontsize=6.5,
    ...
    ax.set_xlabel("X (m)",     color=(0.0, 1.0, 0.5), fontsize=6, labelpad=1)
    ax.set_ylabel("Depth (m)", color=(0.0, 1.0, 0.5), fontsize=6, labelpad=1)
    ax.set_zlabel("Z (m)",     color=(0.0, 1.0, 0.5), fontsize=6, labelpad=1)

    ax.tick_params(colors=(0.0, 0.85, 0.45), labelsize=5)
    ax.xaxis.label.set_color((0.0, 1.0, 0.5))
    ax.yaxis.label.set_color((0.0, 1.0, 0.5))
    ax.zaxis.label.set_color((0.0, 1.0, 0.5))

    # Neon axis lines
    ax.xaxis.line.set_color((0.0, 0.7, 0.35))
    ax.yaxis.line.set_color((0.0, 0.7, 0.35))
    ax.zaxis.line.set_color((0.0, 0.7, 0.35))

    # Tick label colors
    for lbl in ax.get_xticklabels() + ax.get_yticklabels() + ax.get_zticklabels():
        lbl.set_color((0.0, 0.8, 0.4))
        lbl.set_fontsize(5)

    ax.view_init(elev=22, azim=-65)

    # ── Bottom info bar ───────────────────────────────────────────────────────
    ax2 = fig.add_axes([0, 0, 1, 0.10])
    ax2.set_facecolor((0.02, 0.05, 0.09))
    ax2.axis('off')
    n_v     = len(tracks)
    closest = min((t.dist for t in tracks), default=0.0)

    ax2.text(0.02, 0.55,
             f"VEHICLES: {n_v}   CLOSEST: {closest:.1f}m   TRACKS: {n_v}",
...
def process_video(input_path: str):
    ...
    while True:
        ...
        # ── Update tracker ────────────────────────────────────────────────────
        now = time.time()
        fps_display = 0.9*fps_display + 0.1*(1.0/(now-t_prev+1e-6))
        t_prev = now
        ...
        for t in tracks:
            ...
            tscaled.history = deque(
                [(int(x*sx), int(y*sy)) for x,y in t.history], maxlen=30
            )
        ...
        # ─────────────────────────────────────────────────────────────────────
        #  RIGHT PANEL: 3D perception — neon
        # ─────────────────────────────────────────────────────────────────────
